...
/*   Updated: 2021/01/02 18:47:13 by dthan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		get_basic_info(t_l *l, t_auto *auto_com)
{
	auto_com->path_env = get_var("PATH", g_shell.env, VAL);
	auto_com->typed_str = auto_get_typed_str(l);
	auto_com->target_str = auto_get_target_str(auto_com);
}

void		auto_command(t_l *l, t_auto *auto_com, t_editor_phase *phase)
{
	get_basic_info(l, auto_com);
	...
	if (auto_com->count_list == 0)
	{
		...
		auto_cmd_file_multiple_cases(l, auto_com, phase);
	else
	{
