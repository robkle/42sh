...
/*   Updated: 2021/03/13 16:02:39 by dthan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fc_process(t_ft_fc instance)
{
	...
	if (instance.ops & FC_L_OP)
		ret = fc_list(instance.ops, instance.blocks[FIRST], instance.blocks[LAST]);
	else
...
int ft_fc(t_process *p)
{
	...
	if (fc_parse_input(p, &(instance.ops), &(instance.editor), instance.blocks) == EXIT_FAILURE)
	{
		clean_up_ft_fc_struct(instance);
		return (EXIT_FAILURE);
	}
